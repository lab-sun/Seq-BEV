...
import sys  #####
#sys.path.append("..")  #####
#from seqBEV.TemporalFusion import TemporalFusion  #####
sys.path.append("../..")
...
class Xception(nn.Module):
    ...
    """
    def __init__(self, downsample_factor, IF_SHIFT=False):
        """ Constructor
        Args:
        ...
        if self.if_shift:   #####
            self.conv1 = TemporalFusion(nn.Conv2d(3, 32, 3, 2, 1, bias=False), n_segment=self.opt.num_squence, n_div=self.opt.shift_div)   #####
        else:   #####
        ...
        if self.if_shift:   #####
            self.conv2 = TemporalFusion(nn.Conv2d(32,64,3,1,1,bias=False), n_segment=self.opt.num_squence, n_div=self.opt.shift_div)   #####
        else:   #####
...
def xception(pretrained=True, downsample_factor=16, IF_SHIFT=False):
    model = Xception(downsample_factor=downsample_factor, IF_SHIFT=False)
    if pretrained:
        weight_path = '/workspace/pretrained_weight/xception_pytorch_imagenet.pth'

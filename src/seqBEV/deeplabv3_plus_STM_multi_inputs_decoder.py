...
class DeepLab(nn.Module):
    def __init__(self, num_classes, num_classes_layout=4, backbone="mobilenet", pretrained=True, downsample_factor=16):
        ...
        #     nn.Conv2d(32, num_classes, 3, stride=1, padding=1),
        #     nn.BatchNorm2d(num_classes),
        #     nn.ReLU(inplace=True),
        #     nn.Dropout(0.5),

        #     # nn.ConvTranspose2d(num_classes, num_classes, 3, stride=2, padding=0),
        #     # nn.Dropout(0.1)
        # )

    def forward(self, x):
        pre_frame = torch.split(x, self.num_squence, dim=1)[0]
        cur_frame = torch.split(x, self.num_squence, dim=1)[1]
        next_frame = torch.split(x, self.num_squence, dim=1)[2]
        img_seq = [pre_frame, cur_frame, next_frame]
        ...
        #-----------------------------------------#
        low_level_feature_seq = []
        x_feature_seq = []
        for frame in img_seq:
            low_level_features, x = self.backbone(frame)
            # print('in the deeplabv3_plus, high_level_features before aspp: ', x.shape)
            low_level_features = self.shortcut_conv(low_level_features)
            low_level_feature_seq.append(low_level_features)
            x_feature_seq.append(x)
        #-----------------------------------------#
        #   将加强特征边上采样
        #   与浅层特征堆叠后利用卷积进行特征提取
        #-----------------------------------------#
        low_level_features = torch.stack(low_level_feature_seq).sum(dim=0)  # ([bs, 48, 64, 128])
        # print("low_level_features shape after stack: ", low_level_features.shape)
        ...
        x = torch.stack(x_feature_seq).sum(dim=0)
        x = self.aspp(x)

...
# from TemporalShift import TemporalShift
# sys.path.append("../..")
...
from src.seqBEV.TemporalShift import TemporalShift
from configs.opt import get_args
    ...
    def __init__(self, downsample_factor=8, pretrained=True, IF_shift = False):
        ...
        if self.shift:
            for i in self.down_idx:
                ...
                #self.features[i].conv[0] = TemporalShift(self.features[i].conv[0], n_segment=self.opt.num_squence, n_div=self.opt.shift_div)  # 在Expansion layer前做时间融合
                self.features[i].conv[3] = TemporalShift(self.features[i].conv[3], n_segment=self.opt.num_squence, n_div=self.opt.shift_div)  # 在Expansion layer后做时间融合  需要修改预训练参数载入
        # ---------------------------------------------------- #
    ...
    for k, v in pretrained_dict.items():
        ...
        if k in model_dict.keys() and np.shape(model_dict[k]) == np.shape(v):
            if 'backbone' in k:
                ...
                #if int(InvertedResidual_layer) in temporal_layer and int(conv_layer) == 0:   # 时间融合模块位置修改后，也需要修改这里

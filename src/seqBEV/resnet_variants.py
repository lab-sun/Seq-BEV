#############################################resnet for only one image input#######################################################
# import torch
...
#         #output = torch.add(output, output_prepre)
#         output = output_prepre
...
class base_resnet(nn.Module):
    def __init__(self):
        super(base_resnet, self).__init__()
        self.model = models.resnet101(pretrained=True)  # 更换resnet层数 18， 34， 50， 101， 152
        self.model.avgpool = nn.AdaptiveAvgPool2d((1,1))
